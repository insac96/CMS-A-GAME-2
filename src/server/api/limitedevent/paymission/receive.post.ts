import type { IAuth, IDBItem, IDBLimitedEventPayMission, IDBUser } from '~~/types'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Event
    const eventData = await DB.LimitedEventPayMission
    .findOne({ _id: payment })
    .populate({
      path: 'gift.item',
      select: 'item_id type'
    })
    .select('-createdAt -updateAt') as IDBLimitedEventPayMission
    ...
    const paymission = user.limitedevent.paymission
    ...
    eventData.gift.forEach((gift : any) => {
    ...
    // Update User
    await DB.User.updateOne({ _id: auth._id, 'limitedevent.paymission.money': eventData.need }, {
      $set: { 'limitedevent.paymission.$.receive': true }
    })
