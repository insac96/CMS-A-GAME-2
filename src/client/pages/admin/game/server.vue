<template>
  <UiContent title="Server" sub="Thông tin máy chủ trò chơi">
    <UiFlex class="mb-2">
      ...
      <SelectDate v-model="page.range.end" placeholder="Kết thúc" size="sm" class="ml-1 max-w-[140px] mr-auto"/>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('game/admin/server', JSON.parse(JSON.stringify(page.value)))
